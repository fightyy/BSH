...
- `fig2_ITH_public_liver_lung_corrected/`

  Spatial ITH estimates, including raw values and values corrected for tumor purity or sequencing coverage.

  - `x`: x-coordinate of the grid point
  - `y`: y-coordinate of the grid point
  - `ITH_raw`: ITH value before correction
  - `ITH_corrected`: ITH value after purity or coverage correction
  - `correction_cohort`: cohort used to select the correction parameters
  - `correction_model`: correction variable, either tumor purity or sequencing coverage
  - `tumor_type`: tumor type
  - `patient`: patient ID
...
#### fig2_public_liver_lung_location_absolute_nsr_1169_final.csv
...
- `X`: x-coordinate  
- `Y`: y-coordinate  
- Tumor_sample_barcode: A unique identifier for each tumor sample, generated by combining the patient ID and sample ID.
...
#### fig4_real_ITH_corrected_center_edge.csv
...
- `ITH_mean`: mean corrected ITH in the center or edge region. ITH is defined as the average genetic distance between all pairs of samples within a circular area (radius = median pairwise distance of samples in that area).  
